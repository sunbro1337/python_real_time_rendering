...
#from contexts.res_context import collect_sprites
...
class Sprites:
    def __init__(self):
        ...
        self.sprite_parameters = {
            'barrel': {
                'sprite': pygame.image.load('res/sprites/barrel/base/0.png'),
            ...
            'pin': {
                'sprite': pygame.image.load('res/sprites/pin/base/0.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.6,
                'scale': 0.6,
                'animation': deque([pygame.image.load(f'res/sprites/pin/anim/{i}.png').convert_alpha() for i in range(8)]),
                'animation_dist': 800,
                'animation_speed': 10,
                'blocked': True,
            },
            'kakodemon': {
                'sprite': [pygame.image.load(f'res/sprites/kakodemon/base/{i}.png').convert_alpha() for i in range(8)],
                'viewing_angles': True,
                'shift': -0.2,
                'scale': 1.1,
                'animation': deque(
                    [pygame.image.load(f'res/sprites/kakodemon/anim/{i}.png').convert_alpha() for i in range(9)]),
                'animation_dist': 150,
                'animation_speed': 10,
                'blocked': True,
            },
            'flame': {
                'sprite': pygame.image.load('res/sprites/flame/base/0.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.7,
                'scale': 0.6,
                'animation': deque(
